...
const MIGRATION_ID = "20260611_tabforge_referral_tiers_1_2";
...
export async function up(knex) {
  ...
  for (const tier of TABFORGE_TIERS) {
    await knex("reward_queue")
      ...
      .whereRaw("metadata->>'tier_key' = ?", [`verified_signup:${tier.requiredPurchases}`])
      ...
      await knex("referral_programs")
        ...
        .update({
          ...
          metadata: {
            ...(existingProgram.metadata || {}),
            qualification: "verified_signup",
            tiers: [{ requiredPurchases: 5, rewardAmountCents: 1000 }],
            description: "Five verified SendForge account referrals earn a $10 manual Cash App payout. No purchase necessary.",
          },
          updated_at: knex.fn.now(),
        });
    }
