...
authRouter.post("/register", async (req, res) => {
  if (!env.jwtSecret) return res.status(500).json({ error: "JWT_SECRET missing" });
  ...
  try {
    await db("users").insert({ id, email, password_hash: hash });
  } catch {
    return res.status(409).json({ error: "email_in_use" });
  }

  const token = jwt.sign({ sub: id, email }, env.jwtSecret, { expiresIn: "30d" });
  res.json({ token });
});
...
authRouter.post("/login", async (req, res) => {
  ...
  const token = jwt.sign({ sub: u.id, email }, env.jwtSecret, { expiresIn: "30d" });
  res.json({ token });
});
