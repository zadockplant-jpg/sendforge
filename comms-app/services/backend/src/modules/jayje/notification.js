...
export function notificationBody(row, config) {
 ...
 const text = `New JayJe service request — ${row.reference}\n\nName: ${row.name}\nEmail: ${row.email}\nPhone: ${row.phone || 'Not provided'}\nContact by: ${row.contact_method}\nLocation: ${row.location}\nServices: ${selected}\nTimeframe: ${TIMEFRAMES[row.timeframe] || row.timeframe}${row.referral_code?`\nReferral code: ${row.referral_code}`:''}\n\nProject details:\n${row.message}\n\nReply to this email to contact the customer.\n`;
 const fields = [['Name',row.name],['Email',row.email],['Phone',row.phone||'Not provided'],['Contact by',row.contact_method],['Location',row.location],['Services',selected],['Timeframe',TIMEFRAMES[row.timeframe]||row.timeframe],...(row.referral_code?[['Referral code',row.referral_code]]:[])];
